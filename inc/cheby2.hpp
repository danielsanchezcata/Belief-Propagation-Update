...
namespace cheby {
  ...
  class Chebyshev {
    ...
    public:
      ...
      //floatT root(floatT a=-1, floatT b=1, floatT tol=1e-10, int max_its=500);
      floatT root(floatT, floatT, floatT, int);
      ...
      void operator-=(floatT);
      floatT operator()(floatT x) {return Chebyshev_value(x, c);};

      Chebyshev operator-(floatT x) const {
        Chebyshev ans(f - x, INIT_BY_VALUES);
	return ans;
      }
  ...
  floatT Chebyshev::root(floatT a=-1, floatT b=1, floatT tol = 1e-10, int max_its = 500){
    floatT fa = value(a);
    floatT fb = value(b);
    for (int i=0; i<max_its; ++i){
        floatT m = 0.5*(a+b), fm = value(m);
        if(std::fabs(fm) < tol) return m;
